...
export function Admin() {
  ...
  // --- AS PORTARIAS OFICIAIS DO N8N ---
  const URL_PEGA_DADOS = "https://n8n.saintsolution.com.br/webhook/b46feb90-df11-489d-b234-d10de0ff82a6"; 
  ...
  const loadDashboard = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(URL_PEGA_DADOS);
      if (!res.ok) throw new Error("Erro no servidor");
      const data = await res.json();
      const listaRaw = Array.isArray(data) ? data : (data.indicadores || []);
      const precosVindoDoN8n = Array.isArray(data) ? data.filter((item: any) => item.cod_plan) : [];
  ...
  useEffect(() => { loadDashboard(); }, [isAuthenticated]);
  ...
  const handleSavePrecos = async () => {
    setLoading(true);
    try {
      await fetch(URL_ATUALIZA_PRECO, {
        method: 'POST',
  ...
  const handleSaveIndicador = async (dadosvindosdoModal: any) => {
    ...
    const payload = {
      ...
      nome: dadosvindosdoModal.nome || dadosvindosdoModal.formData?.nome || "",
      local: dadosvindosdoModal.local || dadosvindosdoModal.formData?.local || "",
      whatsapp: dadosvindosdoModal.whatsapp || dadosvindosdoModal.formData?.whatsapp || "",
      status: 'ativo'
    };
    ...
    try {
      ...
      if (response.ok) {
        alert("Enviado com sucesso! Aguarde a atualização automática...");
        
        // --- DELAY PARA O GOOGLE SHEETS PROCESSAR A FÓRMULA ---
        setTimeout(async () => {
          await loadDashboard(); 
          setLoading(false);
          setActiveModal(null);
        ...
        alert("O n8n recebeu, mas não salvou. Verifique o nó do Sheets.");
        setLoading(false);
      }
    } catch (err) {
      alert("Erro de conexão. Verifique o CORS no n8n.");
      setLoading(false);
    }
  };
  ...
  const registrarVenda = async (cliente: string, plano: string) => {
    setLoading(true);
    try {
      await fetch(URL_SALVA_INDICADOR, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col lg:flex-row font-sans">
      <aside className="w-full lg:w-72 bg-[#0A2540] p-8 text-white flex flex-col shrink-0 sticky top-0 h-screen">
        ...
        <div className="space-y-3">
          <button onClick={() => { setActiveModal(null); loadDashboard(); }} className={`w-full flex items-center gap-4 p-4 rounded-2xl font-black text-[10px] uppercase transition-all ${!activeModal ? 'bg-[#D4AF37] text-[#0A2540]' : 'text-gray-400 hover:bg-white/5'}`}>
        ...
        <div className="grid gap-4">
          {indicadores.length > 0 ? (
            indicadores.filter(i => i.nome?.toLowerCase().includes(searchTerm.toLowerCase())).map((ind, index) => (
              <div key={ind.id || index} className="bg-white rounded-[32px] p-6 shadow-sm border border-gray-100 flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative group transition-all">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => { setSelectedIndicador(ind); setActiveModal('venda'); }} className="px-6 py-4 bg-green-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 hover:bg-[#0A2540] transition-all"><DollarSign className="w-4 h-4" /> Venda Fechada</button>
                </div>
              </div>
            ))
          ) : (
      ...
      <ModalVenda isOpen={activeModal === 'venda'} onClose={() => setActiveModal(null)} indicadorNome={selectedIndicador?.nome || ''} onConfirm={registrarVenda} />
