...
import { 
  ...
  Loader2, X, ShieldCheck, Save, ArrowLeft, Phone, MapPin, Power, Edit3, LogOut, Lock
} from 'lucide-react';
...
export function Admin() {
  ...
  const carregarDados = async () => {
    setLoading(true);
    try {
      const [resInd, resPre] = await Promise.all([
  ...
  // --- FUNÇÃO CORRIGIDA PARA O SPLIT DO N8N FUNCIONAR ---
  const handleSaveTabelas = async () => {
    try {
      // 1. Transformamos o objeto {E1: 500} em uma LISTA [{cod_plan: 'E1', valor_mensalidade: 500}]
      const payloadParaN8N = Object.keys(precosData).map(cod => ({
        cod_plan: cod,
        valor_mensalidade: precosData[cod]
      }));

      // 2. Enviamos como JSON REAL (sem no-cors, sem plain-text)
      const response = await fetch(LINK_SALVA_PRECOS, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        alert("Sucesso! O n8n recebeu a lista e vai atualizar o Sheets.");
      } else {
        alert("O n8n recebeu, mas o Sheets deu erro. Verifique o mapeamento no n8n.");
      }
    } catch (error) {
      console.error("Erro ao salvar:", error);
      alert("Erro de conexão. Certifique-se que o CORS no n8n está como '*'.");
    }
  };
  ...
  if (!isAuthorized) {
    return (
      <div className="min-h-screen bg-[#0A2540] flex items-center justify-center">
        <form onSubmit={handleAuth} className="bg-white p-12 rounded-[40px] shadow-2xl text-center">
          <Lock className="w-10 h-10 text-[#D4AF37] mx-auto mb-6" />
          <input type="password" placeholder="SENHA" className="w-full p-4 bg-gray-100 rounded-2xl mb-4 text-center font-black" onChange={(e) => setPassword(e.target.value)} />
          <button type="submit" className="w-full bg-[#0A2540] text-white py-4 rounded-2xl font-black uppercase italic">Entrar</button>
        </form>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      <aside className="w-72 bg-[#0A2540] p-8 text-white flex flex-col shadow-2xl fixed h-full z-50">
        <h1 className="text-2xl font-black italic text-[#D4AF37] uppercase mb-10 tracking-tighter text-shadow-sm">SaintSolution</h1>
        <nav className="flex-1 space-y-3">
          <button onClick={() => setActiveTab('dashboard')} className={`flex items-center gap-4 w-full p-5 rounded-2xl font-black text-xs uppercase italic ${activeTab === 'dashboard' ? 'bg-[#D4AF37] text-[#0A2540]' : 'text-white/40'}`}><LayoutDashboard className="w-4 h-4" /> Dashboard</button>
          <button onClick={() => { setSelectedPartner({ id: '', nome: '', whatsapp: '', local: '', status: 'ativo' }); setShowModal('new'); }} className="flex items-center gap-4 w-full p-5 text-white/40 hover:bg-white/5 rounded-2xl font-black text-xs uppercase italic"><UserPlus className="w-4 h-4" /> Novo Parceiro</button>
          <button onClick={() => setActiveTab('precos')} className={`flex items-center gap-4 w-full p-5 rounded-2xl font-black text-xs uppercase italic ${activeTab === 'precos' ? 'bg-[#D4AF37] text-[#0A2540]' : 'text-white/40'}`}><Settings className="w-4 h-4" /> Editar Preços</button>
        </nav>
      </aside>
      ...
      <main className="flex-1 ml-72 p-12 overflow-y-auto">
        ...
        {activeTab === 'precos' && (
          <div className="animate-in slide-in-from-right-10">
            <h2 className="text-5xl font-black text-[#0A2540] mb-12 uppercase italic text-center tracking-tighter leading-tight text-shadow-sm">Tabelas <span className="text-[#D4AF37]">Premium</span></h2>
            <div className="grid md:grid-cols-2 gap-10 mb-10">
              {['E', 'A'].map((prefixo) => (
                <div key={prefixo} className="bg-white p-12 rounded-[48px] shadow-xl border relative overflow-hidden transition-transform hover:scale-[1.01]">
                  ...
                  <h3 className="text-2xl font-black text-[#0A2540] uppercase italic mb-8 text-center tracking-tight">
                      ...
                      return (
                        <div key={cod} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border shadow-inner">
                          ...
                          <input 
                            ...
                            className="bg-white px-4 py-2 rounded-xl border-2 border-gray-100 font-black text-[#0A2540] w-32 text-right outline-none focus:border-[#D4AF37] transition-all" 
                          />
                        </div>
                      );
                    })}
            ...
            <div className="max-w-2xl mx-auto">
              <button 
                ...
                className="w-full bg-[#0A2540] text-[#D4AF37] py-8 rounded-[32px] font-black uppercase italic flex items-center justify-center gap-4 hover:bg-[#D4AF37] hover:text-[#0A2540] transition-all shadow-2xl text-xl active:scale-95 shadow-blue-900/10"
              >
                <Save className="w-8 h-8" /> Gravar Alterações de Preços no Sheets
              </button>
            </div>
          </div>
        )}
          ...
          <form onSubmit={handleSavePartner} className="bg-white w-full max-w-5xl rounded-[50px] p-12 shadow-2xl overflow-y-auto max-h-[95vh] animate-in zoom-in-95 duration-300">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <input type="text" placeholder="LOCALIDADE / BAIRRO" value={selectedPartner?.local || ""} className="p-5 bg-gray-50 rounded-2xl font-bold outline-none border-2 border-gray-100 focus:border-[#D4AF37] placeholder:text-gray-300" onChange={(e) => setSelectedPartner({...selectedPartner, local: e.target.value})} />
              </div>
